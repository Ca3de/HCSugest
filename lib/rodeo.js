...
//
// windowDays: how far back the ExSDRange filter goes. Narrower windows
// reduce "zombie" batches (entries Rodeo still reports as PickingPicked
// even though the physical cart is long gone). Default 3; bumping higher
// gives completeness at the cost of false positives. 14+ days consistently
// over-counts by 2-3x vs floor observation.
async function getBacklogForPath({ warehouse, processPath, windowDays = 3 }) {
  const now = Date.now();
  ...
  const stateBreakdown = { ts_only: 0, pack_ready: 0, rebin_in_progress: 0, packing: 0, mixed: 0, unknown: 0, other: 0, error: 0 };
