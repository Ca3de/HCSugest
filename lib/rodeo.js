...
async function getBacklogForPath({ warehouse, processPath, windowDays = 14 }) {
  ...
  // Classify batches in parallel with bounded concurrency. Rodeo tolerates
  // ~8 concurrent reqs per host comfortably from a single profile.
  async function classify(batchIds, sourceWorkPool, concurrency = 6) {
    ...
    async function worker() {
      while (next < batchIds.length) {
        ...
        try {
          ...
          out[i] = { batchId, items: full.length, units, state, sourceWorkPool };
        } catch (e) {
          out[i] = { batchId, items: 0, units: 0, state: 'error', sourceWorkPool, error: String(e) };
        }
      }
    }
  ...
  const [rebinClassified, packClassified] = await Promise.all([
    classify(rebinBatchIds, 'PickingPicked'),
    classify(packBatchIds,  'Sorted')
  ]);
