...
// Every work pool across the entire item lifecycle. When we re-query a
// batch by PickBatchId we MUST include all of these — otherwise items in
// pools we didn't ask for are silently hidden. E.g. items still being
// picked (PickingNotYetPicked with P-1-* scannables) were invisible,
// causing batches mid-pick to look like "all ts" and count as rebin-ready.
const ALL_WORK_POOLS = [
  // Pre-pick
  'ReadyToPick',
  'PickingNotYetPicked',
  'CrossdockNotYetPicked',
...
function buildBatchUrl({ warehouse, batchId }) {
  ...
  url.searchParams.set('WorkPool', ALL_WORK_POOLS);
...
// Singles: no batch IDs, no rebin. Items go straight from pick to pack.
// A "cart" is a unique Scannable ID with >10 items (smaller groups aren't
// full carts). Query WorkPool=PickingPicked only.
const SINGLES_MIN_ITEMS_PER_CART = 10;

async function getSinglesBacklog({ warehouse, processPath, windowDays }) {
  const now = Date.now();
  const startMs = now - windowDays * 86400000;
  const endMs   = now + 86400000;

  const { rows } = await fetchPoolAllPages({
    warehouse, workPool: 'PickingPicked', processPath, startMs, endMs
  });

  // Group by Scannable ID (not batch ID)
  const byScannableId = Object.create(null);
  for (const r of rows) {
    const sid = r.scannableId;
    if (!sid) continue;
    (byScannableId[sid] ||= []).push(r);
  }

  const packReady = [];
  const excluded  = [];

  for (const [sid, items] of Object.entries(byScannableId)) {
    const units = items.reduce((s, r) => s + (r.quantity || 1), 0);
    if (items.length > SINGLES_MIN_ITEMS_PER_CART) {
      packReady.push({ batchId: sid, items: items.length, units, state: 'pack_ready' });
    } else {
      excluded.push({ batchId: sid, items: items.length, units, state: 'too_small' });
    }
  }

  // Log one sample for the debug tab
  if (self.Debug) {
    await self.Debug.recordSample({
      kind: `rodeo-singles-${processPath}`,
      url: buildPoolUrl({ warehouse, workPool: 'PickingPicked', processPath, startMs, endMs }),
      status: 200,
      body: '',
      parseSummary: {
        totalRows: rows.length,
        uniqueScannables: Object.keys(byScannableId).length,
        qualifyingCarts: packReady.length,
        tooSmall: excluded.length
      }
    });
  }
  ...
  return {
    ...
    stateBreakdown: {
      pack_ready: packReady.length,
      too_small: excluded.length
    },
    totalBatchesSeen: Object.keys(byScannableId).length
  };
}
...
async function getBacklogForPath({ warehouse, processPath, windowDays = 30 }) {
  // Singles have a completely different flow: no batch IDs, no rebin.
  // Cart = unique scannable ID with >10 items. Straight to pack-ready.
  if (processPath === 'PPFracsSingle') {
    return getSinglesBacklog({ warehouse, processPath, windowDays });
  }
