...
//   all rb*, no ts                       -> 'pack_ready'
//   has at least one item in
//     WorkPool=PickingPickedInTransit
//     AND no rb*, no being_picked, no sp -> 'rebin_ready'    (cart en route to wall)
//   anything else                        -> 'mixed'           (excluded)
//
// 'rebin_ready' is the new strict rule per user: PickingPickedInTransit is the
// floor signal for "cart is moving to the rebin wall". All-ts batches without
// the InTransit signal are likely still being staged or stuck and don't count.
function classifyBatch(items) {
  ...
  if (hasInTransit && !hasRb && !hasBeingPicked && !hasSp) return 'rebin_ready';
  return 'mixed';
}
