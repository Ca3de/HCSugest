...
async function fetchFunctionRollup({ warehouseId, processId, spanType, range }) {
  ...
  try {
    ...
    // Richer diagnostics when the parser finds 0 employees, so we can tell
    // "empty FCLM response" from "parser missed something".
    const doc = new DOMParser().parseFromString(html, 'text/html');
    const tableCount = doc.querySelectorAll('table').length;
    const resultTableCount = doc.querySelectorAll('table.result-table').length;
    let amznCells = 0;
    for (const td of doc.querySelectorAll('td')) {
      if (td.textContent.trim() === 'AMZN') { amznCells++; if (amznCells > 100) break; }
    }
    const headings = Array.from(doc.querySelectorAll('h1,h2,h3,h4,b,strong'))
      .map(h => (h.textContent || '').trim()).filter(t => t && t.length < 80).slice(0, 10);
    self.Debug && await self.Debug.recordSample({
      kind, url, status: res.status, body: html,
      parseSummary: {
        employees: employees.length,
        subFunctions: [...new Set(employees.map(e => e.subFunction))],
        diagnostics: { tableCount, resultTableCount, amznCells, sampleHeadings: headings }
      }
    });
    return employees;
  } catch (e) {
