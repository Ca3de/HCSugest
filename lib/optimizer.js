...
function aaHoursDemand({ unitsBacklog, cartsBacklog, expectedUPH, role }) {
  // role: 'pick' | 'rebin' | 'pack'
  if (role === 'pick') {
    return unitsBacklog / Math.max(1, expectedUPH);
  }
  if (role === 'rebin') {
    // TODO: derive carts/AA/hr from historical FCLM Rebin sub-function UPH
    //       divided by observed avg units-per-cart. Placeholder: 1.5 carts/hr.
    const cartsPerAAHr = 1.5;
    return cartsBacklog / cartsPerAAHr;
  }
  if (role === 'pack') {
    // TODO: same as rebin — learn cartsPerPackerHr from history.
    const cartsPerAAHr = 2.0;
    return cartsBacklog / cartsPerAAHr;
  }
  return 0;
}
